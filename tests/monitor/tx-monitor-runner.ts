...
async function run() {  
  ...
  mp.reply().subscribe(async (task) => {
    ...
    await request('http://localhost:3002/rx-txjs/monitor', { json: true }, (err, response, body) => {
        ...
        logger.info('(subscribe) monitor server is ready on http://localhost:3002/rx-txjs/monitor: ', task);        
      })
  });  
