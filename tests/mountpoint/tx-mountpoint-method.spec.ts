...
describe('Mount Point Class', () => {
  ...
  it('tx-mountpoint-method.spec: check mountpoint methods callback', (done) => {
    ...
    class C1Component {      
      ...
      constructor() {
        logger.info("C1Component:constructor is called, no need to subscribe, the method will take care of it");            
        this.mountpoint.tasks().method('run', this);
        this.mountpoint.tasks().method('more', this);
      }
      
      run(task: TxTask<any>) {
        logger.info("[C1Component:run] is called .. task = ", task.get());
      ...
      more(task: TxTask<any>) {
        logger.info("[C1Component:more] is called .. task = ", task.get());
        expect(task.data.from).to.equal('https://api.github.com');
