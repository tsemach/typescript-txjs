...
export class A3Component {
  mountpoint = TxMountPointRegistry.instance.create('GITHUB::GIST::A3');
  ...
  constructor() {
    this.mountpoint.tasks().subscribe(
      (task) => {
        logger.info('[A3Component:tasks] got task = ' + JSON.stringify(task, undefined, 2));          
        this.method = task['method'];

        // just send the reply to whom is 'setting' on this reply subject
        this.mountpoint.reply().next(new TxTask({method: 'from A3', status: 'ok'}, task['data']))
      },
      (error) => {
        ...
        // just send the reply to whom is 'setting' on this reply subject
        this.mountpoint.reply().error(new TxTask({method: 'from A3', status: 'ERROR'}, error['data']))
      }
    );
