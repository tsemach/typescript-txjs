...
export class A3Component {
  ...
  constructor() {
    this.mountpoint.tasks().subscribe(
      ...
      (error) => {
        logger.info('[A3Component:error] got error = ' + JSON.stringify(error.get(), undefined, 2));
    ...
    this.mountpoint.undos().subscribe(
      (task) => {
          logger.info('[A3Component:undo] undo got task = ' + JSON.stringify(task.get(), undefined, 2));
          this.method = task['method'];
